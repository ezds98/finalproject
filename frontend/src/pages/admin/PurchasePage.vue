<template>
    <main class="mx-auto max-w-7xl py-8 px-4 sm:px-6 lg:px-8 lg:pb-24 bg-white">
        <div class="max-w-xl">
            <h1 class="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">My Orders history</h1>
        ...
        <section aria-labelledby="recent-heading" class="mt-8">
            ...
            <div v-if="allOrders.length > 0" class="space-y-20">
                <div v-for="order in allOrders" :key="order._id">
                    ...
                    <table class="mt-4 w-full text-gray-500 sm:mt-6">
                        ...
                        <tr v-for="prod in order.products" :key="prod.product[0]._id">
                            <td class="py-6 pr-8">
                                <div class="flex items-center">
                                    <UploadImageView image-class="mr-6 h-16 w-16 rounded object-cover object-center"
                                                     :img-id="prod.product.images && Array.isArray(prod.product[0].images) && prod.product[0].images.length > 0?prod.product[0].images[0]:null"></UploadImageView>
                                    
                                    <div>
                                        <div class="font-medium text-gray-900">{{ prod.product[0].name }}</div>
                                        <div class="mt-1 sm:hidden">{{ prod.product[0].price }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="hidden py-6 pr-8 sm:table-cell">${{ prod.product[0].price }}</td>
                            ...
                            <td class="whitespace-nowrap py-6 text-right font-medium ">
                                <a :href="'/product/details/'+prod.product[0]._id" class="text-indigo-600"
            ...
            <div v-else>
                <AlertComponent heading="No Orders Found" details="Please let any other user purchase your created books to continue" />
            </div>
        </section>
    </main>
</template>
...
function getOrders() {
    api.getData("sales/get/user/orders").then((resp) => {
        if (!resp.error) {
            allOrders.splice(0);
